...
export function PopularSearchTerms() {
  return (
    <div className="glass-card rounded-2xl p-6 mb-6">
      <h3 className="font-dm font-semibold text-lg mb-4">Popüler Arama Terimleri</h3>
      <div className="space-y-3">
        {popularSearchTerms.map((item, i) => (
          <div
            key={i}
            className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-surface-card rounded-xl hover:bg-primary-light/30 transition-colors"
          >
            <div className="flex items-center gap-3 min-w-0">
              <span className="text-accent font-bold">#{i + 1}</span>
              <span className="text-sm text-gray-300 break-words">{item.term}</span>
            </div>
